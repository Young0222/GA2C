...
class EmbeddingEvaluation():
	...
	def ee_multioutput_binary_classification(self, train_emb, train_y, val_emb, val_y, test_emb, test_y):
		...
		params_dict = {'multioutputclassifier__estimator__C': [1e-1, 1e0, 1e1, 1e2]}
...
# 			params_dict = {'alpha': [500, 50, 5, 0.5, 0.05, 0.005, 0.0005]}
	...
	def svc_classify(self, x, y, search):
		...
		for train_index, test_index in kf.split(x, y):
			...
			if search:
				...
				# params = {'C':[1e-4, 5e-4, 1e-3, 5e-3, 1e-2, 5e-2, 1e-1, 5e-2, 1, 5, 1e1, 5e1, 1e2, 5e2, 1e3, 5e3, 1e4]}
	...
	def evaluate_embedding(self, embeddings, labels, search=True):
		...
		_acc_val, _acc = self.svc_classify(x,y, search)
		if _acc_val > acc_val:
			acc_val = _acc_val
			acc = _acc
	...
	def kf_embedding_evaluation_autogcl(self, encoder, dataset, folds=10, batch_size=128):
		data_loader = DataLoader(dataset, batch_size=batch_size, shuffle=False)
		embeddings, labels = get_emb_y(data_loader, encoder, self.device, is_rand_label=self.is_rand_label)
		acc, _ = self.evaluate_embedding(embeddings, labels)
