...
def joint_cl_exp(
    ...
    for fold, (train_idx, test_idx, val_idx, semi_idx) in enumerate(
        ...
        # model = GInfoMinMax(TUEncoder(num_dataset_features=dataset.num_features, emb_dim=128, num_gc_layers=3, drop_ratio=0.0, pooling_type='layerwise'), 128).to(device)
        model = model_func(dataset).to(device)
        view_learner_actor = ViewLearner(TUEncoder(num_dataset_features=dataset.num_features, emb_dim=128, num_gc_layers=3, 
                            drop_ratio=0.0, pooling_type='layerwise'), mlp_edge_model_dim=128).to(device)
        view_learner_critic = ViewLearner(TUEncoder(num_dataset_features=dataset.num_features, emb_dim=128, num_gc_layers=3, 
                            drop_ratio=0.0, pooling_type='layerwise'), mlp_edge_model_dim=128).to(device)
        optimizerA = torch.optim.Adam(view_learner_actor.parameters(), lr=0.001)
        optimizerC = torch.optim.Adam(view_learner_critic.parameters(), lr=0.001)
        optimizer = Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
        ...
        for epoch in range(1, epochs + 1):
            model_loss, actor_loss, critic_loss = train_node_weight_view_gen_and_cls(
                                                    epoch, view_learner_actor, view_learner_critic, optimizerA, optimizerC,
                                                    model, optimizer,
                                                    semi_loader, device)
            ...
            logger.info('Epoch: {:03d}, Model Loss: {:.4f}, Actor Loss: {:.4f}, Critic Loss: {:.4f}, '
                        'Train Acc: {:.4f}, Test Acc: {:.4f}'.format(epoch, model_loss, actor_loss, critic_loss, train_acc, test_acc))
...
def train_node_weight_view_gen_and_cls_orig(view_gen1, view_gen2, view_optimizer, 
...
def train_node_weight_view_gen_and_cls(epoch, view_learner_actor, view_learner_critic, optimizerA, optimizerC,
                                        model, model_optimizer, dataloader, device):
    num_T = 3
    model_loss_all = 0
    actor_loss_all = 0
    critic_loss_all = 0
    ...
    for batch in dataloader:
        ...
        if epoch % num_T == 1:
            ...
            for i in range(num_T):  # batch_aug_edge_weight is important!
                ...
                x_aug = model(batch, batch_aug_edge_weight)
                # Dist & Value
        ...
        batch.y = torch.squeeze(batch.y)
        loss1 = F.nll_loss(x, batch.y)
        loss2 = F.nll_loss(x_aug, batch.y)
        cls_loss = (loss1 + loss2) / 2
        cl_loss = loss_cl(x, x_aug)
        model_loss = model_loss + cls_loss + cl_loss
        # 2.b. standard gradient descent formulation
        model_loss.backward()
        model_optimizer.step()

    # now = t()
    # logging.info('current time: {}; total time: {}'.format(now-prev, now-start))
    # prev = now
